    ...
    public final function execute($query, &$num_rows=-1, $return=true) {
        ...
        while($retries>=0){
            ...
            if ($result === false) { //there was an error on the query, or we are not connected to the db
                ...
                if($this->link==null || !mysqli_ping($this->link)){
        ...
        if($this->link==null){ //are we connected to the DB???
            throw new Exception("Se ha perdido la conexión a la BBDD.");
            ...
            if ($result === true) { //Query was an insert,update or delete
                $num_rows = mysqli_affected_rows($this->link);
                if ($return) {
                    ...
                    $rid=mysqli_query($this->link,'SELECT last_insert_id() lid');
                    $aid=mysqli_fetch_assoc($rid);
                    $id = $aid['lid'];
